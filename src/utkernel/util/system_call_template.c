...
inline static bool IsNotError(int return_code) { return return_code == kNonsenseParameter || return_code >= 0; }
...
static int Execute(const char* system_call_name, ExecuteDelegate execute, const void* info) {
  int id = its_id;

...
inline static INT TemplateWithoutId(const char *system_call_name,
...
ID tk_cre_tsk(CONST T_CTSK *pk_ctsk) {
  return TemplateWithoutId(__func__, _tk_cre_tsk, pk_ctsk);
}
inline static INT Template(const char *system_call_name, ID tskid,
                           ExecuteDelegate execute, const void *info) {
...
ER tk_sta_tsk(ID tskid, INT stacd) {
  return Template(__func__, tskid, _tk_sta_tsk, NULL);
}
...
ER tk_slp_tsk(TMO tmout) {
  return TemplateWithoutId(__func__, _tk_slp_tsk, &tmout);
}
...
ER tk_dly_tsk(RELTIM dlytim) {
  return TemplateWithoutId(__func__, _tk_dly_tsk, &dlytim);
}
